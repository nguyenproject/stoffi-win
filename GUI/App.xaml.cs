	...
	public partial class App : Application
	{
		...
		App()
		{
			...
			if (!Directory.Exists(languageFolder))
				languageFolder = Path.Combine(baseFolder, @"..\..\Languages");
		...
		private void ArgumentsReceived(Object state)
		{
			...
			try
			{
				...
				Dispatcher.BeginInvoke(DispatcherPriority.Normal, new Action(delegate() { ((Windows.StoffiWindow)MainWindow).CallFromSecondInstance(argument); }));
			}
			catch { }
		}
		...
		private void Uninstall()
		{
			try
			{
				var filetypes = new Stoffi.Player.GUI.Windows.Associations().FullFileList;
		...
		/// </summary>
		/// <param name="all">If true than all supported filetypes will be associated, otherwise the user will get to choose.</param>
		...
		private void Application_DispatcherUnhandledException(object sender, DispatcherUnhandledExceptionEventArgs e)
		{
			...
			{
				var stoffi = Application.Current.MainWindow as Windows.StoffiWindow;
