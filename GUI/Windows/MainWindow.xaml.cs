...
namespace Stoffi.Player.GUI.Windows
{
	...
	public partial class StoffiWindow : Window, INotifyPropertyChanged
	{
		...
		{
			if (!SettingsManager.FirstRun) return;
			U.L(LogLevel.Information, "Main", "This is the first run, need to setup some stuff");
			Stoffi.Core.Sources.Radio.Manager.PopulateDefaults();
			Dispatcher.Invoke(DispatcherPriority.Background, new Action(delegate()
			{
				// Show welcome dialog
			...
			ThreadStart GUIScanThread = delegate()
			{
				try
				{
					...
					lock (addTrackLock)
					{
						...
						if (tracksToAdd.Count > 0 || tracksToRemove.Count > 0)
						{
